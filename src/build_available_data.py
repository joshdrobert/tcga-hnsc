...
def main() -> None:
    ...
    parsed.to_csv(processed / "tcga_hnsc_clinical_xml_flat.csv", index=False)
    analysis = build_tcga_analysis_table(parsed, "TCGA-HNSC")
    analysis.to_csv(processed / "tcga_hnsc_analysis_cohort.csv", index=False)
    write_summary_tables(analysis, tables)
    print(f"Wrote TCGA-HNSC cohort: {len(analysis):,} patients")
